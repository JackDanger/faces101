...
$('#user-email, #email').blur(function() {
  var $parentContainer = $(this).parents('fieldset').length? $(this).parents('fieldset') : $(this).parents('form');
  var $errorBox = $('.message-box-error', $parentContainer);
  var hasErrorBox = $errorBox.length;
  var $errorList = $('ul', $errorBox);
  var hasErrorList = $errorList.length;
  var wrapperElement = hasErrorList ? '<li/>' : '<div/>';
  var $parentElement = hasErrorList ? $errorList : $errorBox;
  var hideErrorBoxWhenFixed = $errorBox.children().length == 0 || $errorList.children().length == 0;
  $(this).mailcheck({
    suggested: function($element, suggestion) {
      var emailDomainSuggestion = $(wrapperElement, {
        'class': 'domain-suggestion',
        'html': 'Did you mean <a href="#update" class="domain-suggestion-address">' + suggestion.address + '@<strong class="domain">' + suggestion.domain + '</strong></a>?',
        'click': function (e) {
          $(this).remove();
          if (hideErrorBoxWhenFixed) {
            $errorBox.removeAttr('style');
            $("body").removeClass("page-error");
          };
          $element.val(suggestion.full);
          $element.parent('.field-error').removeClass('field-error');
          e.preventDefault();
        }
      });
      $('.domain-suggestion', $parentContainer).remove();
      $parentElement.prepend(emailDomainSuggestion);
      $errorBox.show();
      $element.parent('.field').addClass('field-error');
...
(function() {
  ...
  (function($) {
    ...
    Threshold = (function() {
      ...
      Threshold.parseThresholds = function(el, hash) {
        var dimensions, key, thresholds;
        dimensions = {
          top: el.offset().top,
        ...
        for (key in hash) {
          if (hash.hasOwnProperty(key)) {
            thresholds.push(new Threshold(key, dimensions, hash[key]));
          }
        }
        return thresholds;
      };

      function Threshold(key, dimensions, callback) {
        this.callback = callback;
        this.callback || (this.callback = function() {});
        this.parseThresholdKey(key, dimensions);
      }

      Threshold.prototype.parseThresholdKey = function(key, dimensions) {
        var direction, edge, offset, _ref;
        _ref = key.split(":"), edge = _ref[0], offset = _ref[1], direction = _ref[2];
        offset = parseInt(offset, 10);
        if (edge === 'bottom') {
          return this.y = dimensions.bottom + offset;
        } else {
          this.y = dimensions.top + offset;
          return this.direction = direction === 'down' ? 'down' : 'up';
        }
      };

      Threshold.prototype.check = function(start, end, winHeight) {
        var y;
        y = this.y - winHeight;
    ...
    return $.fn.inViewport = function(_options) {
      var defaults, options, w, winHeight;
      w = $(window);
      defaults = {
        enter: function() {},
        exit: function() {},
      ...
      winHeight = w[0].innerHeight;
      options = $.extend(defaults, _options);
      w.resize(function() {
        return winHeight = w[0].innerHeight;
      });
      return this.each(function() {
        var elem, lastScrollTop, offsetTop, thresholds;
        elem = $(this);
        offsetTop = elem.offset().top;
        thresholds = Threshold.parseThresholds(elem, options.thresholds);
        lastScrollTop = w.scrollTop();
        return w.scroll(function() {
          var scrollTop, threshold, _i, _len;
          scrollTop = w.scrollTop();
          if ((scrollTop + winHeight) > offsetTop && (offsetTop + elem.height()) > scrollTop) {
          ...
          for (_i = 0, _len = thresholds.length; _i < _len; _i++) {
            threshold = thresholds[_i];
            threshold.check(lastScrollTop, scrollTop, winHeight);
          }
...
$.fn.labelFader = function() {
    var focusClass = "fade-label-focus";
    var f = function() {
      var $this = $(this);
      var $labelContent = $this.siblings("label").children("span");
      if ($this.val()) {
        $labelContent.hide();
      } else {
        $labelContent.fadeIn("fast");
      }
    };
    this.focus(function() {
      f.call(this);
      $(this).parents(".field").addClass(focusClass);
    });
    this.blur(function() {
      f.call(this);
      $(this).parents(".field").removeClass(focusClass);
    });
    this.keyup(f);
    this.keydown(f);
    this.change(f);
    this.each(function() {
      ...
      if ($label.children().length == 0) {
        var $span = $('<span>').text($label.text()).css('display','block').fadeIn();
...
// Google AdWords conversion helpers
(function($) {
  $.trackAwConversion = function(label) {
    // Overload document.write to append to an element in the page rather than
    // replacing the whole page.
    document.write = function(content) {
      if (!$("#gaw_content").length) {
        $("<div>").attr("id","gaw_content").appendTo("body");
      }
      $('#gaw_content').html(content);
    };

    google_conversion_id = 984344943;
    google_conversion_language = "en";
    google_conversion_format = "3";
    google_conversion_color = "666666";
    google_conversion_value = 0;
    google_conversion_label = label;
    $.getScript(document.location.protocol + '//www.googleadservices.com/pagead/conversion.js');
  };
})(jQuery);


// Google Analytics, and Square Analytics helpers
var _gaq = _gaq || [];
var _saq = _saq || [];
(function($) {
  $.trackPageView = function(path) {
    ...
    _saq.push(['PageView', document.location.protocol + '//' + document.location.host + path]);
  };

  $.trackEvent = function(category, action, label, value) {
    if(!category || !action || !_gaq) return;
    _gaq.push(['_trackEvent', category, action, label, value]);
    _saq.push([category+'-'+action, label]);
  };
})(jQuery);
...
$(document).ready(function() {
  ...
  $('a.track-event, button.track-event').click(function() {
    var link_name = this.getAttribute("name");
    $.trackEvent("link", "click", link_name, 1);
  });

  $('input.track-event').click(function() {
    var input_value = this.getAttribute("value");
    $.trackEvent("button", "click", input_value, 1);
  });

  $('.video-link, .play-button').click(function(){
    var video_name = $(this).attr('data-video-name');
    $.trackEvent("videos", "play", video_name, 1);
  });
});
var SquareAnalytics = function(_application_name) {
  this.application_name = _application_name;

  this.urlsFor = function(events) {
    var eventsToRecord = [];
    var metaDataPresent = false;
    var self = this;

    $.each(events, function(index, fcEvent){
      if (fcEvent.length == 2) {
        eventsToRecord.push(fcEvent);
      }
      if (fcEvent.length == 3) {
        metaDataPresent = true;
        if(typeof(fcEvent[2]) == 'string'){
          fcEvent[2] = {'data':fcEvent[2]};
        }
        eventsToRecord.push(fcEvent);
      }
      if (fcEvent.visitor){
        self.visitor = fcEvent.visitor;
      }
    });
    ...
    var url = baseUrl;
    var self = this;
    $.each(eventsToRecord, function(index, eventToRecord){
      var newFragment = '&n[]=' + encodeURIComponent(eventToRecord[0]) + '&v[]=' + encodeURIComponent(eventToRecord[1]);
      if (metaDataPresent){
        newFragment += self.encodeMetadata(eventToRecord, index);
      }
      // Group fragments by 2048 - baseUrl.length
      if (url.length + newFragment.length > 2048){
        urls.push(url);
        url = baseUrl;
      }
      url += newFragment;
  ...
  this.encodeMetadata = function(fcEvent, index){
    if (fcEvent.length == 2){
      return "&m[" + index + "]=";
    } else {
      var encodedMetadata = '';
      $.each(fcEvent[2], function(key, value){
        encodedMetadata += "&m[" + index + "][" + encodeURIComponent(key) + "]=" + encodeURIComponent(value);
      });
  ...
  this.baseUrl = function(){
    var queryString = [];
    var baseUrl = SquareAnalytics.BASE_URL + "t?";

    if (this.application_name) {
      queryString.push("app_name=" + this.application_name);
    }
    queryString.push(SquareAnalytics.ANONYMOUS_VISITOR_TOKEN_PARAM + '=' + encodeURIComponent(this.getCookieValue()));
    if (this.visitor) {
      queryString.push(SquareAnalytics.IDENTIFIED_VISITOR_TOKEN_PARAM + '=' + encodeURIComponent(this.visitor));
    }
  ...
  this.track = function(events) {
    // if no UUID is set populate the uuid before tracking the data.
    var self = this;
    if (!this.getCookieValue()) {
      $.getJSON(SquareAnalytics.BASE_URL + "uuid?callback=?", function(data) {
        self.setCookieValue(data.uuid);
        SquareAnalytics.createTrackingPixels(self.urlsFor(events));
      });
    } else {
      SquareAnalytics.createTrackingPixels(this.urlsFor(events));
    }
  };
  ...
  this.setCookieValue = function(value) {
    ...
    return (document.cookie = [encodeURIComponent(SquareAnalytics.COOKIE), '=', encodeURIComponent(value)].join('') + ";expires=" + now.toUTCString());
  };
  ...
  this.getCookieValue = function() {
    ...
    for (i=0;i<cookies.length;i++) {
      ...
      var name = cookieData[0].replace(/^\s+|\s+$/g,"");
      if (name == SquareAnalytics.COOKIE) {
...
SquareAnalytics.createTrackingPixels = function(trackingUrls) {
  $.each(trackingUrls, function(index, trackingUrl){
    new Image().src = trackingUrl;
  });
};

SquareAnalytics.trackPageLoad = function() {
  var now = new Date().getTime();
  if (window.performance && window.performance.timing) {
    var totalPageLoad = now - window.performance.timing.navigationStart;
    _saq.push(['PageLoad', document.location, {'totalPageLoadTime': totalPageLoad}]);
  }
};

var squareAnalyticsURL = squareAnalyticsURL || 'https://sa.squareup.com/';

SquareAnalytics.BASE_URL = squareAnalyticsURL;
SquareAnalytics.COOKIE = "_savt";
SquareAnalytics.IDENTIFIED_VISITOR_TOKEN_PARAM = "ivt";
SquareAnalytics.ANONYMOUS_VISITOR_TOKEN_PARAM = "avt";

var EventQueue = function(squareAnalytics) {
  this.squareAnalytics = squareAnalytics;

  this.push = function(arg) {
    this.squareAnalytics.track([arg]);
  };
};

// Example usage:
// At the top of page, or before pushing an event
// var _saq = _saq || [];
// Push your events
// _saq.push(['eventName', 'eventValue']);
// _saq.push({'visitor': '<%= user.token %>'});
// Record all events
var squareAnalytics = new SquareAnalytics(_saq_app_name);
var _saq = _saq || [];

// track_page_view
_saq.push(["PageView", document.location, {'ref_url': document.referrer}]);
squareAnalytics.track(_saq);

_saq = new EventQueue(squareAnalytics);

$(window).load(function(){
  SquareAnalytics.trackPageLoad();
});
...
;(function ($, window, undefined) {
  var scrollElem = setScrollableElement('html', 'body'),
  ...
  function setScrollableElement(els) {
    var i = 0,
        argsLength = arguments.length,
        $scrollElement, isScrollable, el;
  ...
  function setScrollDuration (delta) {
    var minFrameRefresh = 17,
        maxPixelsPerFrame = 100,
        maxPixelsPerSecond = 50 * 1000 / 17;

    return (delta / maxPixelsPerSecond) * 1000;
  }
    ...
    return this.each(function (i, element) {
      ...
      if ( locationPath == thisPath
        ...
        && this.hash.replace(/#/,'') ) {
          ...
          targetOffset = target && $target.length ? $target.offset().top : null;
      }

      $(this).bind('click', function (e) {
        var scrollDuration = setScrollDuration(Math.abs($(window).scrollTop() - targetOffset));

        // Prevent jump-down
        e.preventDefault();
        // adding this block to combat bug in Safari 6 (on OSX 10.8)
        // where the initial set isn't finding a scrollable element, thus returning an empty array
        if (!scrollElem.length) {
          scrollElem = setScrollableElement('html', 'body');
        }

        // Animate to target
        if (scrollElem.length) {
          $(scrollElem).stop().animate({scrollTop: targetOffset}, scrollDuration, function () {
            location.hash = target;
          });
        } else {
          // fallback for when scrollableElem returns an empty array
          $('html, body').stop().animate({ scrollTop: targetOffset }, scrollDuration, function () {
            location.hash = target;
          })
        }
      });
...
// Locale selector
// Simple selected locale checker
if($('body').hasClass('locale-en-CA')){
  $('.selected-locale').text('Canada (EN)');
  $('.locale[data-locale="ca-en"]').addClass('active').siblings().removeClass('active');
}
else if($('body').hasClass('locale-fr-CA')){
  $('.selected-locale').text('Canada (FR)');
  $('.locale[data-locale="ca-fr"]').addClass('active').siblings().removeClass('active');
}
else {
 $('.selected-locale').text('United States');
 $('.locale[data-locale="us-en"]').addClass('active').siblings().removeClass('active');
}

// Show prompt
$('.selected-locale').click(function(e){
  var $prompt = $(this).parent();
  $prompt.hasClass('active') ? $prompt.removeClass('active') : $prompt.addClass('active');
  e.preventDefault();
});

// Activate new locale
$('.locale').click(function(){
  // Setup vars and cleanup pathname
  var $this = $(this),
      locale = $this.data('locale'),
      region = $this.data('country'),
      localePath = $this.data('path'),
      pathName = window.location.pathname.replace(/^\/(ca(\/fr)?$|ca(\/fr)?\/)/, '/'); // Regex clears ^/ca$, ^/ca/, ^/ca/fr$, and ^/ca/fr/ from path


  // Handle styling changes
  $this.addClass('active').siblings().removeClass('active');
  $('.selected-locale').text(region);
  $('.locale-selector:not(.activation-locale-selector)').removeClass('active');

  // Set locale cookie
  document.cookie = "locale=" + locale + ';path=/';

  // Redirect to respective locale page
  if(localePath == 'us'){
    window.location.href = window.location.protocol + "//" + window.location.hostname + pathName;
  }
  else {
    window.location.href = window.location.protocol + "//" + window.location.hostname + '/' + localePath + pathName;
  }
});
...
/* base.js - includes things that are necessary for the load of EVERY page */


$(function() {
  $(".fade-label input").labelFader();
  $('.create-user .field-error input').live('keydown',function() {$(this).parent('.field-error').removeClass('field-error'); });
  $('.create-user').submit(function(e) {
    ...
    $.ajax({
      ...
      error: function(request) {
        var errorResponse = $.parseJSON(request.responseText);
        $(".field").removeClass("field-error");
        $(".message-box-error", $form).hide().children().remove();
        var msg = "";
        $.each(errorResponse.errors, function(fieldName, message) {
          $("[name='user[" + fieldName + "]']", $form).parent(".field").addClass("field-error");
          if (fieldName == 'password') {
            $("[name='user[password_confirmation]']", $form).parent(".field").addClass("field-error");
          }
          msg += message + '. ';
        });
        var $errorElement = $('<div class="signup-error" />').text(msg);
        $(".message-box-error", $form).append($errorElement).show();
        $('.field-error:first', $form).find('input').focus();
        $("body").addClass("page-error");
  ...
  // use plugin for smooth scroll instead
  $('a.scroll-to, a.open-signup-drawer').smoothScrollTrigger();

  // Homepage Drawer
  var $navDrawer = $('.nav-drawer');
  var $drawerContent = $('.drawer-content');
  slideDrawer = function(show) {
    if ($(show).is(':visible')) {
      $navDrawer.slideUp(function(){
        $drawerContent.hide();
      });
    } else if ($drawerContent.is(':visible')) {
      $navDrawer.slideUp(function(){
        $drawerContent.hide();
        $(show).show();
        $navDrawer.slideDown();
    ...
    $("input[name='user[email]']", $(show)).focus();
  }
  ...
  // SETUP AUTHENTICITY TOKENS FOR ANY CACHED FORMS
  // authenticity token
  var token = $('head meta[name="csrf-token"]').attr('content');
  $('form input[name="authenticity_token"]').each(function(i,e){
     $(e).val(token);
  });
